module.exports = {
  SM: "o SM tankou de mana shield, só levou miss, deu teleport 20x e matou @vc",
  BK: "o BK fez um combo no meio do dash e deu HK em @vc a 2km de distância",
  FE: "a FE equipou o aruan e a asa 3 e matou @vc",
  EMG: "o MG de energia deu uma cocada de bypass em @vc e ele morreu",
  SMG: "o MG de força deu HK em @vc e morreu de reflect pra uma SU aleatória que estava farmando afk",
  MG: "@vc utilize EMG para energia e SMG para força",
  DL: "o DL farmou um full 10, botou batota, pendant solid, asa power e morreu pra @vc",
  SU: "a SU matou todo de reflect, incluve @vc",
  RF: "o RF engatou uma sequência de bundada e deu HK em @vc",
  GL: "a GL secou seu AG, @vc parou de atacar, morreu e xingou a staff",
  RW: "a RW soltou uma bolinha matou @vc e a staff bufou em +3%",
  SL: "a SL passou a faca em @vc e matou, passou o mouse em cima de uma elfa inocente por perto e matou também, voltou a farmar hunt",
  COMANDOS:
    "Classes que você pode trocar pvp: SM, BK, FE, EMG (energia), SMG (força), DL, SU, RF, GL, RW, SL. Ex.: !pvp <classe>",
};
